...
def read_point_cloud(file_path, use_color, return_bounds=False, *args, **kwargs):
    ...
    colors = np.asarray(pcd.colors, dtype=np.float32) if use_color else np.zeros_like(points)
    minb, maxb = pcd.get_min_bound(),  pcd.get_max_bound()
    del pcd
    if return_bounds:
        return points, colors, minb, maxb 
    else:
